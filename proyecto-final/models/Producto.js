...
const math = mathjs.create(mathjs.all, {}) //! no me funcionaba con el import
...
class Producto {
  ...
  async save(newObj) {
    try {
      ...
      const idx = objArr.findIndex(
        obj => newObj.nombre.toLowerCase().trim() === obj.nombre.toLowerCase().trim()
      )
      ...
      await _saveFile.call(this, objArr)
        .then(msg => { Utils.success(`Se ha guardado el producto '${newObj.nombre}' en el archivo ${this.fileName}`) })
  ...
  async update(newObj) {
    try {
      ...
      // Agrego el timestamp por modificación
      newObj.timestamp = Date.now()
      ...
      return await _saveFile.call(this, objArr)
        .then(msg => {
          Utils.success(`Se ha actualizado el producto '${obj.nombre}' en el archivo ${this.fileName}`)
          return obj
        })
        .catch(err => Utils.error(`Ha ocurrido un error al escribir el archivo ${this.fileName}.${err.message}`))
